...
const HEAD_TOP = 0.33;
/** DÃ³nde van los lentes respecto del hueso de la cabeza: a la altura de los ojos, delante de la cara. */
...
type Anim = 'idle' | 'walk' | 'sprint' | 'pick-up' | 'emote-yes' | 'holding-both';
...
export class Character {
  ...
  async setLook(look: Look) {
    ...
    if (token !== this.loadToken) return; // llegÃ³ otro cambio mientras cargaba
    ...
    this.actions.clear();
    this.current = null;
    for (const name of ['idle', 'walk', 'sprint', 'pick-up', 'emote-yes', 'holding-both'] as Anim[]) {
  ...
  /** AnimaciÃ³n de una vez: recoger un volantÃ­n o celebrar un corte. */
  playOnce(name: 'pick-up' | 'emote-yes') {
    this.current = null;
    this.play(name, 0.15, true);
  }

  /**
   * Ubica al personaje, lo orienta y elige la animaciÃ³n.
   * Con `kite` sostiene el hilo: el brazo derecho apunta al volantÃ­n.
   */
  update(pos: V3, speed: number, kite: V3 | null, facing: number, dt: number) {
    ...
    if (kite && !this.oneShot && this.armR && this.armL) {
      ...
      // El brazo izquierdo sostiene el tarro mÃ¡s abajo, hacia el volantÃ­n
  ...
  /** PosiciÃ³n de la mano derecha en el mundo: el ancla del hilo. */
